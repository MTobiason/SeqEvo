...
public class referencePosition
{
   ...
   private static TreeMap<Integer,Long> intraScoreMap = new TreeMap<Integer,Long>();
   private static TreeMap<Integer,Long> interScoreMap = new TreeMap<Integer,Long>();
   ...
   public static void initializeReferencePositions(Generation IG)
   {
      ...
      for(int i = 1; i <= tempLSL; i++) // for every length up to the longest strand length.
      {
         ...
         long tempScore = tempOL.entrySet().stream().mapToLong(e-> e.getValue()*scoringFunction(e.getKey())).sum();
   ...
   */
               ...
               if(complementLength != 0)
               {
                  if (tempIsIntramolecular)
                  {
                     if (complementLength >= intramolecularSLC)
                     {
                        ...
                        tempScore = tempScore.add(BigInteger.valueOf(intraScoreMap.get(complementLength)));
                     }
                  }
                  else
                  {
                     ...
                     {
                        //record the complement
                        tempScore = tempScore.add(BigInteger.valueOf(interScoreMap.get(complementLength)));
                     }
                  }
                  complementLength = 0;
               }
   ...
   private static long scoringFunction (int IL ) //incoming length
   {
      long tempScore = 1;
